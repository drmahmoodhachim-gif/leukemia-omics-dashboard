...
export function suggestGroup(char: string): SampleGroup {
  ...
  if (/control|healthy|normal|donor|hsc|cd34\+|remission|cr\b|complete remission/.test(c)) return "A";
  if (/aml|all|cll|leukemia|blast|relapse|diagnosis|patient|case|disease|tumor|malignant/.test(c)) return "B";
  if (/pre-chemo|baseline|diagnosis/.test(c)) return "B";
  if (/post-chemo|survivor|recovery/.test(c)) return "A";
  return null;
}
