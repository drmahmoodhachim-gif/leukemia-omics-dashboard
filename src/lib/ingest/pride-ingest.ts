...
const KEYWORDS = [
  "acute myeloid leukemia proteomics",
  "AML bone marrow mass spectrometry",
  "acute lymphoblastic leukemia proteomics",
  "CLL proteome",
  "leukemia phosphoproteomics",
];
...
function inferOmics(_desc: string): string {
  return "proteomics";
}
...
function inferTissue(title: string, desc: string): string {
  ...
  if (t.includes("bone marrow")) return "bone_marrow";
  if (t.includes("pbmc") || t.includes("peripheral blood")) return "pbmc";
  if (t.includes("blast")) return "blast";
...
async function searchPride(
  ...
  const res = await fetchWithRetry(`${PRIDE_BASE}/search/projects?${params}`);
