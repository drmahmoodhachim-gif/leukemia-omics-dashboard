...
CREATE OR REPLACE FUNCTION leukemia_omics.bulk_upsert_publications(data jsonb)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION leukemia_omics.bulk_upsert_datasets(data jsonb)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = leukemia_omics
...
CREATE OR REPLACE FUNCTION leukemia_omics.upsert_ingest_manifest(
...
GRANT EXECUTE ON FUNCTION leukemia_omics.bulk_upsert_publications(jsonb) TO service_role;
GRANT EXECUTE ON FUNCTION leukemia_omics.bulk_upsert_datasets(jsonb) TO service_role;
GRANT EXECUTE ON FUNCTION leukemia_omics.upsert_ingest_manifest(timestamptz, int, jsonb, jsonb, text) TO service_role;
